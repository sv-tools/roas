//! Version newtype for AsyncAPI v3.1 (`asyncapi: "3.1.x"`).
//!
//! Constrained to `3.1.<patch>` with an optional `-suffix`
//! ([JSON Schema](https://asyncapi.com/schema-store/3.1.0.json)).
//! Deserialization rejects non-3.1 values up front, so a 2.6 or 3.0
...
const VERSION_SCHEMA_DESCRIPTION: &str = "`3.1.<patch>` with optional `-suffix` (AsyncAPI v3.1)";

/// Pattern `^3\.0\.\d+(-.+)?$` — hand-rolled to avoid a regex engine for
/// one check.
fn matches_asyncapi_3_0_version(s: &str) -> bool {
    let Some(rest) = s.strip_prefix("3.1.") else {
        return false;
    };
...
mod tests {
    ...
    #[test]
    fn default_is_3_1_0() {
        assert_eq!(Version::default().as_str(), "3.1.0");
    }

    #[test]
    fn accepts_matching_versions() {
        assert!("3.1.0".parse::<Version>().is_ok());
        assert!("3.1.7".parse::<Version>().is_ok());
        assert!("3.1.0-rc1".parse::<Version>().is_ok());
    }

    #[test]
    fn rejects_non_matching_versions() {
        for bad in ["2.6.0", "3.0.0", "4.0.0", "3.1", "3.1.x", "3.1.0-", ""] {
    ...
    fn display_renders_inner_string() {
        let v: Version = "3.1.3".parse().unwrap();
    ...
    fn try_from_str_and_string_match_from_str() {
        ...
        let owned_ok = Version::try_from(String::from("3.1.9")).unwrap();
        assert_eq!(owned_ok.as_str(), "3.1.9");
