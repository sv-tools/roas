//! AsyncAPI v3.1 `Schema` and `Multi Format Schema` objects.
//!
//! Per [Schema Object](https://www.asyncapi.com/docs/reference/specification/v3.1.0#schemaObject)
//! and [Multi Format Schema Object](https://www.asyncapi.com/docs/reference/specification/v3.1.0#multiFormatSchemaObject).
//!
...
/// The `schemaFormat` values AsyncAPI 3.1 documents as "formats tooling
/// MUST support".
///
/// Identical to the 3.0 list plus this version's own dialect
/// (`…asyncapi;version=3.1.0` and its `+json` / `+yaml` twins). Note
/// that the OpenAPI and Avro entries stay pinned to their 3.0-era
/// versions, and 3.1's payload dialect still accepts every 2.x and
/// 3.0.0 format.
///
