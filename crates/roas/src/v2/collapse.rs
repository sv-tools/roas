//! `Spec::collapse` for OAS 2.0 — lift the reusable inline `Schema`,
//! `Parameter`, and `Response` values into the top-level
//! `definitions`, `parameters`, and `responses` bags.
//!
...
mod tests {
    ...
    fn trivial_property_schemas_stay_inline_while_objects_lift() {
        ...
        assert!(category.starts_with("#/definitions/"), "got {category}");
        // …and the lifted object's own scalar property stays inline too.
        let lifted = schema_at(&spec, category.trim_start_matches("#/definitions/"));
        assert_eq!(
