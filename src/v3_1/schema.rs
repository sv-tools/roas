...
use crate::common::helpers::{validate_pattern, Context, PushError, ValidateWithContext};
    ...
    fn validate_with_context(&self, ctx: &mut Context<Spec>, path: String) {
        ...
        if let Some(properties) = &self.pattern_properties {
            for (pattern, schema) in properties {
                let path = format!("{}.pattern_properties[{}]", path, pattern);
            ...
            match unevaluated_properties {
                ...
                BoolOr::Item(schema) => {
                    schema.validate_with_context_boxed(
                        ctx,
                        format!("{}.unevaluatedProperties", path),
                    );
                }
            }
        }
        ...
        if let Some(property_names) = &self.property_names {
            property_names.validate_with_context_boxed(ctx, format!("{}.propertyNames", path));
        }
    }
}
...
impl ValidateWithContext<Spec> for MultiSchema {
    fn validate_with_context(&self, ctx: &mut Context<Spec>, path: String) {
        ...
        let mut unique_types: HashSet<String> = HashSet::with_capacity(self.types.len());
        self.types.iter().for_each(|t| {
            if !allowed_types.contains(t) {
                ctx.error(
                    format!("{}.type", path),
                    format_args!("type `{}` is not supported", t),
                );
            }
            if !unique_types.insert(t.clone()) {
                ctx.error(
                    format!("{}.type", path),
                    format_args!("type `{}` is not unique", t),
                );
            }
        });
        if let Some(docs) = &self.external_docs {
...
mod object_set_type {
    ...
    pub(super) fn deserialize<'de, D>(_: D) -> Result<String, D::Error>
    where
        D: Deserializer<'de>,
    {
        Ok("object".into())
    ...
    pub(super) fn serialize<S>(_: &String, serializer: S) -> Result<S::Ok, S::Error>
    where
        S: Serializer,
    {
...
mod tests {
    ...
    fn test_string_serialize_deserialize() {
        let spec = Schema::Single(SingleSchema::String(StringSchema {
    ...
    fn test_integer_serialize_deserialize() {
        let spec = Schema::Single(SingleSchema::Integer(IntegerSchema {
            title: Some("foo".to_string()),
    ...
    fn test_number_serialize_deserialize() {
        let spec = Schema::Single(SingleSchema::Number(NumberSchema {
            title: Some("foo".to_string()),
    ...
    fn test_boolean_serialize_deserialize() {
        let spec = Schema::Single(SingleSchema::Boolean(BooleanSchema {
            title: Some("foo".to_string()),
            default: Some(false),
    ...
    fn test_array_serialize_deserialize() {
        let spec = Schema::Single(SingleSchema::Array(ArraySchema {
            title: Some("foo".to_string()),
            items: Some(BoolOr::Item(Schema::new_boxed_single_schema_ref(
                SingleSchema::Integer(IntegerSchema {
                    title: Some("bar".into()),
                    ..Default::default()
                }),
            ))),
    ...
    fn test_object_serialize_deserialize() {
        let spec = Schema::Single(SingleSchema::Object(ObjectSchema {
            title: Some("foo".to_string()),
            properties: Some(BTreeMap::from_iter(vec![(
                "bar".into(),
                Schema::new_boxed_single_schema_ref(
    ...
    fn test_null_serialize_deserialize() {
        let spec = Schema::Single(SingleSchema::Null(NullSchema {
            title: Some("foo".to_string()),
    ...
    fn test_multi_serialize_deserialize() {
        let spec = Schema::Multi(MultiSchema {
            types: vec!["string".into(), "integer".into()],
            title: Some("foo".to_string()),
    ...
    fn test_one_of_serialize_deserialize() {
        let spec = Schema::OneOf(OneOfSchema {
            one_of: vec![
    ...
    fn test_any_of_serialize_deserialize() {
        let spec = Schema::AnyOf(AnyOfSchema {
            any_of: vec![
                RefOr::new_ref("#/components/schemas/Cat".into()),
                RefOr::new_ref("#/components/schemas/Dog".into()),
                RefOr::new_ref("#/components/schemas/Lizard".into()),
    ...
    fn test_all_of_serialize_deserialize() {
        let spec = Schema::AllOf(AllOfSchema {
            all_of: vec![
                RefOr::new_ref("#/components/schemas/Cat".into()),
                RefOr::new_ref("#/components/schemas/Dog".into()),
                RefOr::new_ref("#/components/schemas/Lizard".into()),
