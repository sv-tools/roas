...
pub enum Parameter {
    ...
    /// `application/x-www-form-urlencoded`). MUST NOT appear more than
...
/// Holds a parameter with `in: querystring` property (added in OAS 3.2).
/// Carries the entire URL query string as a single value via `content`.
/// `name`'s value is not used in serialization but the field is required
/// by the Parameter Object schema.
#[derive(Clone, Debug, Deserialize, Serialize, PartialEq, Default)]
pub struct InQuerystring {
    /// **Required** Per the spec, `name` is required even though its
    /// value is unused for serialization of `in: querystring` parameters.
    pub name: String,
...
impl ValidateWithContext<Spec> for Parameter {
    fn validate_with_context(&self, ctx: &mut Context<Spec>, path: String) {
        match self {
            ...
            Parameter::Querystring(p) => p.validate_with_context(ctx, path),
...
impl ValidateWithContext<Spec> for InQuerystring {
    fn validate_with_context(&self, ctx: &mut Context<Spec>, path: String) {
        validate_required_string(&self.name, ctx, format!("{path}.name"));
        either_example_or_examples(ctx, &self.example, &self.examples, path.clone());
        // Spec: `content` is required; the map MUST contain exactly one
        // media-type entry.
        if self.content.is_empty() {
            ctx.error(
                path.clone(),
                "must define `content` (style/explode/schema are not allowed for `in: querystring`)",
            );
