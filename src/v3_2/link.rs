...
/// Resolve an internal operationRef. Per OAS 3.2, an operationRef is a
...
fn resolve_internal_operation_ref(spec: &Spec, reference: &str) -> OperationRefResolution {
    ...
    while consumed < parts.len() {
        ...
        // JSON Pointer tokens are case-sensitive; OAS 3.2.0 fixes Operation
...
mod tests {
    ...
    fn operation_ref_into_components_path_items() {
        // Per OAS 3.2, operationRef can target an Operation in
        // `#/components/pathItems/<name>/<method>`.
    ...
    fn operation_ref_method_token_is_case_sensitive() {
        // OAS 3.2.0 fixes Operation field names to lowercase, and JSON
